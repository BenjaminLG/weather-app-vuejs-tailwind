...
const handleSearch = () => {
  ...
  searchTerm.timeout = setTimeout(async() => {
    if (searchTerm.query !== '') {
      ...
      const res = await fetch(`http://api.weatherapi.com/v1/search.json?key=0c10700cabb24719b6b200201242505&q=${searchTerm.query}`);
...
<template>
  <section>
    <!-- search field -->
    <form>
      <article class="bg-white border border-indigo-600/30 rounded-lg shadow-lg flex items-center">
    ...
    <!-- search suggestions -->
    <article class="bg-white my-2 rounded-lg shadow-lg">
